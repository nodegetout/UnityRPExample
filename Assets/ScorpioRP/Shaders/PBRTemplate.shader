Shader "Scorpio/Physically-Based-Lighting"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            VertexOutput vert(VertexInput v)
            {
                ...
                o.tangentDir = normalize(TransformObjectToWorld(v.tangent.xyz));
            ...
            float BeckmannNormalDistribution(float roughness, float NdotH)
            {
                ...
                return max(0.000001,
                           (1.0 / (3.1415926535 * roughnessSqr * NdotHSqr * NdotHSqr)) * exp(
                               (NdotHSqr - 1) / (roughnessSqr * NdotHSqr)));
            }
            ...
            float WardAnisotropicNormalDistribution(float anisotropic, float NdotL, float NdotV, float NdotH,
            ...
            float DuerGeometricShadowingFunction(float3 lightDirection, float3 viewDirection, float3 normalDirection,
            ...
            float BeckmanGeometricShadowingFunction(float NdotL, float NdotV, float roughness)
            {
                ...
                float SmithL = calulationL < 1.6
                                   ? (((3.535 * calulationL) + (2.181 * calulationL * calulationL)) / (1 + (2.276 *
                                       calulationL) + (2.577 * calulationL * calulationL)))
                                   : 1.0;
                float SmithV = calulationV < 1.6
                                   ? (((3.535 * calulationV) + (2.181 * calulationV * calulationV)) / (1 + (2.276 *
                                       calulationV) + (2.577 * calulationV * calulationV)))
                                   : 1.0;
                float Gs = (SmithL * SmithV);
                return Gs;
            }
            ...
            float4 frag(VertexOutput i) : SV_Target
            {
                ...
                normalDirection = shiftAmount < 0.0f
                                      ? normalDirection + viewDirection * (-shiftAmount + 1e-5f)
                                      : normalDirection;
                ...
                // UnityGI gi = GetUnityGI(_LightColor0.rgb, lightDirection, normalDirection, viewDirection,
                //                         viewReflectDirection, attenuation, 1 - _Glossiness, i.posWorld.xyz);
